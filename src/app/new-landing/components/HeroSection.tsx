...
export default function HeroSection() {
  ...
  return (
    <section style={{ position:'relative', minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', paddingTop:80 }}>
      ...
      <div style={{ position:'relative', maxWidth:1280, margin:'0 auto', padding:'0 1.5rem', display:'grid', gridTemplateColumns:'1fr', gap:48, alignItems:'center' }} className="nl-hero-grid">
        <div style={{ display:'flex', flexDirection:'column', gap:32 }}>
          <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} transition={{duration:0.6}}
            style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'0.5rem 1rem', borderRadius:999, background:'hsl(245 60% 28% / 0.1)', border:'1px solid hsl(245 60% 28% / 0.2)', width:'fit-content' }}>
            <Zap size={16} color="hsl(30,80%,55%)" />
            <span style={{ fontSize:'0.875rem', fontWeight:500, color:'hsl(245,60%,28%)' }}>No sugarcoating. Just hard facts.</span>
          </motion.div>
          ...
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.6,delay:0.5}}
            ...
            <motion.button onClick={openModal} whileHover={{scale:1.05}} whileTap={{scale:0.95}}
              style={{ background:grad, color:'hsl(40,33%,97%)', padding:'1rem 2rem', borderRadius:999, fontWeight:700, border:'none', cursor:'pointer', boxShadow:'0 0 60px -15px hsl(245 80% 65% / 0.4)' }}>
              Get Your Report Now →
            </motion.button>
            <motion.button onClick={(e) => { e.preventDefault(); document.getElementById('problem')?.scrollIntoView({ behavior: 'smooth' }); }} whileHover={{scale:1.05}} whileTap={{scale:0.95}}
              style={{ border:'2px solid hsl(240 20% 8% / 0.2)', color:'hsl(240,20%,8%)', padding:'1rem 2rem', borderRadius:999, fontWeight:600, cursor:'pointer', background:'transparent' }}>
      ...
      <style>{`@media(min-width:1024px){.nl-hero-grid{grid-template-columns:1fr 1fr!important}}`}</style>
    </section>
  );
}

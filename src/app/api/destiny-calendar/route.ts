  ...
  try {
    ...
    if (earlyTargetId) {
      const { data: existing } = await supabaseAdmin
        ...
        .eq("report_type", "destiny_window").limit(1).maybeSingle();
      if (existing) {
        console.log("[DESTINY] ✅ Returning existing saved report (0 credits)");
    ...
    if (!profileId || profileId === "self") {
      const { data: fp } = await supabaseAdmin.from("family_profiles").select("*").eq("user_id", user.id).eq("relationship", "Self").maybeSingle();
      if (fp) {
      ...
      const { data: fp } = await supabaseAdmin.from("family_profiles").select("*").eq("id", profileId).maybeSingle();

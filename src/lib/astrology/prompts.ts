...
export function buildClaudeContext(chart: GoldenMasterJSON, personName = "User", topic: JyotishTopic = "general", gocharSnapshot?: GocharSnapshot): string {
  ...
  const planetTable = d1.planets.map((p: PlanetData) => {
    const flags = [
      ...
      p.isCombust    ? "Combust"     : "",
      p.avastha      ? `Avastha:${p.avastha}` : "",
      p.isMrityuBhaga ? "MB(Fatal)" : "",
      p.isPushkaraBhaga ? "PB(Blessing)" : "",
      p.isPushkaraNavamsa ? "PN(Nourished)" : "",
      (ex.lajjitadiAvasthas && ex.lajjitadiAvasthas[p.name] && ex.lajjitadiAvasthas[p.name].length > 0) ? `Lajjitadi:[${ex.lajjitadiAvasthas[p.name].join("/")}]` : ""
    ].filter(Boolean).join(",");
